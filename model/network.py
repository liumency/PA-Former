...
from .modules import TransformerDecoder, Transformer, SqueezeBodyEdge
...
class CDNet(nn.Module):
    def __init__(self, n_class=2, backbone='resnet18', output_stride=16, img_chan=3, f_c = 64):
        ...
        self.backbone = build_backbone(backbone, output_stride, BatchNorm, img_chan, f_c)

        self.SqueezeBodyEdge1 = SqueezeBodyEdge(f_c, BatchNorm)
        self.SqueezeBodyEdge2 = SqueezeBodyEdge(f_c, BatchNorm)
    ...
    def forward(self, img1, img2):
        ...
        body1, out1_s16 = self.backbone(img1)
        body2, out2_s16 = self.backbone(img2)

        x16 = torch.cat([out1_s16, out2_s16], dim=1)

        # +body
        body1 = self.SqueezeBodyEdge1(body1)
        body2 = self.SqueezeBodyEdge1(body2)

        # body = torch.cat([body1, body2], dim=1)
        # x16 =x16+body
